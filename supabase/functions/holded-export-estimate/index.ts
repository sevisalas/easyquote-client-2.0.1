...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Get conditional-hide settings: prompts hidden when value matches a target
    ...
    // Hide-when-value map: productId:KEY -> normalized target value
    ...
    console.log('🙈 Hide-when-value prompts:', Array.from(hideWhenValueMap.entries()));
    ...
    filteredItems.forEach((item: any) => {
      ...
      if (itemHasMultiQuantities) {
        ...
        let totalPrice = 0;
        let units = 1;
        // When true, totalPrice already includes item_additionals,
        ...
        if (storedItemPrice > 0) {
          ...
          // Fallback only when item.price is empty (legacy/incomplete data)
    ...
    return new Response(
      JSON.stringify({
        error: 'Failed to export estimate to Holded'
      }),
      {
        headers: { ...corsHeaders, 'Content-Type': 'application/json' },
        status: 500
