-- Fix next_document_number to properly sync and always increment correctly
-- The issue is that when v_existing_max > last_number, the UPDATE was setting last_number
-- to GREATEST(last_number + 1, v_existing_max + 1) which equals v_existing_max + 1,
-- but THEN subsequent calls would just do last_number + 1 from the OLD value if
-- the row wasn't actually updated due to Postgres optimization.
...
BEGIN
  ...
  -- Calculate next number: max of (current_last + 1, existing_max + 1, 1)
  v_next := GREATEST(
    COALESCE(v_current_last, 0) + 1,
    COALESCE(v_existing_max, 0) + 1,
    1
  );

  -- Upsert the sequence with the calculated next value
