...
-- Service-role-only variant of next_document_number for portal B2B autoservice
CREATE OR REPLACE FUNCTION public.next_document_number_internal(
  p_organization_id uuid,
  p_document_type text
)
RETURNS TABLE(document_number text, sequential_number integer)
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF p_document_type = 'quote' THEN
    ...
    WHERE q.organization_id = p_organization_id
      ...
      AND (NOT v_use_year OR extract(year from q.created_at::timestamptz) = v_year);
  ELSE
    ...
    WHERE o.organization_id = p_organization_id
      ...
      AND (NOT v_use_year OR extract(year from o.created_at::timestamptz) = v_year);
  END IF;

  SELECT last_number INTO v_current_last
  FROM public.document_sequences
  WHERE organization_id = p_organization_id
    AND document_type = p_document_type
    AND year = v_year_bucket
  FOR UPDATE;

  v_next := GREATEST(COALESCE(v_current_last, 0) + 1, COALESCE(v_existing_max, 0) + 1, 1);

  INSERT INTO public.document_sequences (organization_id, document_type, year, last_number)
  VALUES (p_organization_id, p_document_type, v_year_bucket, v_next)
  ON CONFLICT (organization_id, document_type, year)
  DO UPDATE SET last_number = v_next, updated_at = now();
...
REVOKE ALL ON FUNCTION public.next_document_number_internal(uuid, text) FROM PUBLIC;
REVOKE ALL ON FUNCTION public.next_document_number_internal(uuid, text) FROM anon;
REVOKE ALL ON FUNCTION public.next_document_number_internal(uuid, text) FROM authenticated;
GRANT EXECUTE ON FUNCTION public.next_document_number_internal(uuid, text) TO service_role;
